          ...
          for (var index in keywords) {
            ...
            if (dcconPos != -1) {
              message = message.replace(
                new RegExp("~(" + keyword.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + ")([HGVX]*)", 'ig'),
                function(m, dcconKeyword, dcconAdditionalStyle) {
                  ...
                  var dcconStyle = '';
                  var specialtyle = [], additionalTags = '';
                  ...
                  for(var s=0; s<dcconAdditionalStyle.length; s++) {
                    ...
                    switch(dcconStyle) {
                      ...
                      case 'V':
                        ...
                        break;
                      case 'G':
                        specialStyle.push('filter: grayscale();');
                        break;
                      case 'X':
    ...
    lines.forEach( function(element) {
      ...
      // 트위치 명령어 처리
        ...
        case "PRIVMSG":     // 채팅 수신
          ...
          // 클립 링크 파싱
          let clip = null;
          if((clip = /(?:https?:\/\/)?(?:www\.)?twitch\.tv\/[a-zA-Z0-9_]+\/clips?\/([a-zA-Z]+)(.*)?/g.exec(message)) != null) {
            message = message.replace(clip[0], 'https://clips.twitch.tv/' + clip[1]);
          }
          if((clip = message.match(/(https?:\/\/)?clips\.[a-zA-Z0-9./]*/ig)) != null) {
            message = message.replace(clip[0], "");

            if (configData.loadClipPreview) {
              var clipRequest = new window.XMLHttpRequest();
              clipRequest.open('GET', 'https://tv.telk.kr/getclip.php?url=' + encodeURIComponent(clip[0]), true);

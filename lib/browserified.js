...
addChatMessage = function(nick, message, data, msgid) {
  ...
  if (data.msgid) { chatOuterBox.classList.add("msgid_"+data.msgid); }
...
var deleteChatMessage = function(msgid) {
  var children = document.getElementsByClassName("msgid_" + msgid);
  if (children && children.length > 0) {
    for (var index in children) {
      if (isNaN(Number(index))) { continue; }
      children[index].lower.msg.innerHTML = 
...
// 파라미터로 pre-theme 구현
let _params = String(location.href.split('?')[1]).split('&');
for(i in _params) {
  let _tmp = _params[i].split('=');
  let _key = _tmp[0];
  let _val = _tmp[1];
  if(_key === 'theme') {
    configData.theme = _val;
    break;
  }
}
...
var loadCss = function(isRetrying) {
  ...
  document.head.appendChild( function() {
    ...
    ret.rel = "stylesheet";
    ret.href = configData.themeURI + configData.theme + ".css";
          ...
          for (var index in keywords) {
            ...
            if (dcconPos != -1) {
              message = message.replace(
                new RegExp("~(" + keyword.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + ")([BCHGVX]*)", 'ig'),
                function(m, dcconKeyword, dcconAdditionalStyle) {
                  var dcconBackgroundImage = [];
                      dcconBackgroundImage.push('url(' + configData.dcConsURI + dcConsSubURI + dcConsData.find( function(element) { return element.keywords.indexOf(keyword) != -1; } ).name + ')');
                  var specialStyle = [];
                  var filterStyle = [];
                  var dcconStyle = '';
                  var additionalTags = '';
                  ...
                  for(var s=0; s<dcconAdditionalStyle.length; s++) {
                    var dcconStyle = dcconAdditionalStyle.charAt(s).toUpperCase();
                    switch(dcconStyle) {
                      case 'B':
                      ...
                      case 'G':
                        ...
                        break;
                      case 'X':
                        dcconBackgroundImage.push('url(/bbcc/images/X.png)');
                        break;
                      case 'C':
                        dcconBackgroundImage.push('url(/bbcc/images/Censored.png)');
                        break;
                      default:
                        break loop;
                    }
                  }
                  if(flip.h && flip.v) specialStyle.push('transform: scale(-1, -1);');
                  else if(flip.h)      specialStyle.push('transform: scaleX(-1);');
                  else if(flip.v)      specialStyle.push('transform: scaleY(-1);');
                  ...
                  return '<div class="dccon" style="background-image: ' + dcconBackgroundImage.join(',') + '; ' + (specialStyle.length > 0 ? ' ' + specialStyle.join(' ') : '') + '" />' + additionalTags + '</div>';
                }
              );
            }
          }
    ...
    lines.forEach( function(element) {
      ...
      // 트위치 명령어 처리
        ...
        case "PRIVMSG":     // 채팅 수신
          ...
          // 클립 링크 파싱
          let clip = clipId = null;
          if((clip = /(?:https?:\/\/)?(?:www\.)?twitch\.tv\/[a-zA-Z0-9_]+\/clips?\/([a-zA-Z\-_0-9]+)(.*)?/ig.exec(message)) !== null
          || (clip = /(?:https?:\/\/)?clips\.twitch\.tv\/([a-zA-Z\-_0-9]+)(.*)?/ig.exec(message)) !== null)
          {
            clipId = clip[1];
          ...
          if(clipId !== null) {
            if (configData.loadClipPreview) {
              ...
              clipRequest.open('GET', 'https://tv.telk.kr/clip/' + encodeURIComponent(clipId), true);
              console.log('https://tv.telk.kr/clip/' + encodeURIComponent(clipId));

              var handler = function(evt) {
                if(clipRequest.readyState == 4) {
                  if(clipRequest.status == 200) {
                    var src, title, uploader;
                    var clipdata = JSON.parse(clipRequest.responseText);
                    data["clip"] = '<div class="chat_clip_box"><img src="' + clipdata.src +  '" />'
                          + '<div class="chat_clip_title">' + clipdata.title + '</div>'
                          + '<div class="chat_clip_by">' + clipdata.uploader + '</div></div>';
                  } else {
                    data["clip"] = '<div class="chat_clip_box invalid">잘못된 클립입니다!</div>';
              ...
              console.log(clip);
              data["clip"] = '<div class="chat_clip_box text_only">' + configData.clipReplaceMsg + '</div>';
            }
          }

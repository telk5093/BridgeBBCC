          ...
          for (var index in keywords) {
            ...
            if (dcconPos != -1) {
              message = message.replace(
                new RegExp("~(" + keyword.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + ")([HGV]*)", 'ig'),
                function(m, dcconKeyword, dcconAdditionalStyle) {
                  ...
                  return '<img class="dccon" src="' + configData.dcConsURI + dcConsSubURI + dcConsData.find( function(element) {
                    return element.keywords.indexOf(keyword) != -1;
                  } ).name + '"' + (specialStyle.length > 0 ? ' style="' + specialStyle.join('') + '"' : '') + ' />';
                }
              );
            }
          }
